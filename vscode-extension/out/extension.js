...
class DemojibakelizadorProvider {
    constructor() {
...
class EncodingIssue extends vscode.TreeItem {
    constructor(label, filePath, encoding, confidence, collapsibleState) {
...
async function onDocumentOpen(document) {
...
async function detectFileEncoding(document) {
...
function updateStatusBar(status, encoding) {
    const config = vscode.workspace.getConfiguration('demojibakelizador');
    ...
    try {
        await vscode.window.withProgress({
            ...
            if (config.get('autoBackup')) {
                args.push('-backup-suffix', config.get('backupSuffix'));
            }
...
async function scanWorkspace() {
    ...
    try {
        await vscode.window.withProgress({
            ...
            const config = vscode.workspace.getConfiguration('demojibakelizador');
            const extensions = config.get('fileExtensions');
    ...
    for (const line of lines) {
        if (line.includes('WARN') && line.includes('|')) {
            ...
            if (parts.length >= 3) {
                ...
                const issue = new EncodingIssue(fileName, filePath, encoding, confidence, vscode.TreeItemCollapsibleState.None);
                issues.push(issue);
            }
        }
    }
...
async function runDemojibake(args) {
    const config = vscode.workspace.getConfiguration('demojibakelizador');
